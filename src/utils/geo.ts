 ...
 *
 * Every profile gets a `geo` block with an approximate latitude/longitude.
 * Points are drawn around the state capital anchor with an urban/rural
 * spread, clamped inside a generous state bounding box, so a point always
 * lands in its own state (district-level polygons are not bundled, and the
 * docs say so plainly).
 *
 * Runs on an isolated per-profile stream: zero impact on seeded output.
 */

import { gaussianSample } from '../core/sampler.js';
import type { GeoPoint, SeededRNG } from '../types.js';

interface StateGeo {
  /** [lat, lon] of the state capital (well-known anchor) */
  anchor: [number, number];
  /** Generous [minLat, maxLat, minLon, maxLon]; absent for tiny scattered UTs */
  bbox?: [number, number, number, number];
  /** Fixed spread override (km) for tiny UTs without a bbox */
  spreadKm?: { urban: number; rural: number };
}
...
const STATE_GEO: Record<string, StateGeo> = {
  ...
  himachal_pradesh: { anchor: [31.1, 77.17], bbox: [30.2, 33.3, 75.4, 79.1] },
  jammu_kashmir: { anchor: [34.08, 74.8], bbox: [32.2, 37.1, 73.8, 80.4] },
  ...
  odisha: { anchor: [20.3, 85.83], bbox: [17.8, 22.6, 81.3, 87.6] },
...
const FALLBACK: StateGeo = { anchor: [21.0, 78.0], bbox: [6.5, 37.5, 68.0, 97.5] };

/** Bounding box for a state id (null for tiny scattered UTs). Exported for tests. */
export function stateGeoBounds(stateId: string): [number, number, number, number] | null {
  return STATE_GEO[stateId]?.bbox ?? null;
}

/** Anchor point for a state id (capital). Exported for tests. */
export function stateGeoAnchor(stateId: string): [number, number] {
  return (STATE_GEO[stateId] ?? FALLBACK).anchor;
}
 ...
 */
export function generateGeo(
  stateId: string,
  areaType: 'urban' | 'rural',
  rng: SeededRNG
): GeoPoint {
  const g = STATE_GEO[stateId] ?? FALLBACK;
  const [lat0, lon0] = g.anchor;

  let sigmaKm: number;
  if (g.spreadKm) {
    sigmaKm = areaType === 'urban' ? g.spreadKm.urban : g.spreadKm.rural;
  } else if (g.bbox) {
    const [minLat, maxLat, minLon, maxLon] = g.bbox;
    const diagKm = Math.hypot((maxLat - minLat) * 111, (maxLon - minLon) * 111 * Math.cos((lat0 * Math.PI) / 180));
    sigmaKm = Math.max(2, diagKm / (areaType === 'urban' ? 20 : 6));
  } else {
    sigmaKm = areaType === 'urban' ? 4 : 15;
  }

  const lat = lat0 + gaussianSample(0, sigmaKm / 111, rng);
  const lon = lon0 + gaussianSample(0, sigmaKm / (111 * Math.cos((lat0 * Math.PI) / 180)), rng);

  if (g.bbox) {
    const [minLat, maxLat, minLon, maxLon] = g.bbox;
    return {
      latitude: round4(Math.min(maxLat, Math.max(minLat, lat))),
      longitude: round4(Math.min(maxLon, Math.max(minLon, lon))),
    };
  }
  return { latitude: round4(lat), longitude: round4(lon) };
}
